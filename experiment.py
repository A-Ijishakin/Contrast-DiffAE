...
class LitModel(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs, gen= False, pred=True, LAWA=False, double_cond=False) -> None:
        ...
        val_loader = torch.utils.data.DataLoader(ExampleDataset(N=17, val=True), shuffle=False, batch_size=batch_size)
        ...
        if self.current_epoch >= self.conf.load_in:
            with torch.no_grad():
                for idx in range(2):
                    ...
                    loader = train_loader if idx == 0 else val_loader
                    
                    tensors = torch.zeros((1, 512)) 
                    
                    if mode == 'train':
                        train_labels = torch.zeros((1)) 
                    else:    
                        val_labels = torch.zeros((1))  

                    for data in loader:
                        if mode == 'train':
                            train_embeddings = self.model.encoder(data['img'].to('cuda'))
                            tensors, train_labels = torch.concatenate((tensors, train_embeddings.to('cpu')), dim = 0), torch.concatenate((train_labels, data['label'].to('cpu')), dim = 0)
                            
                        else:
                            val_embeddings = self.model.encoder(data['img'].to('cuda'))
                            tensors, val_labels = torch.concatenate((tensors, val_embeddings.to('cpu')), dim = 0), torch.concatenate((val_labels, data['label'].to('cpu')), dim = 0)
                        
                    if mode == 'train':
                        train_tensors = tensors[1:] 
                        affinity_matrix = self.calc_cosine(train_tensors, train_tensors) 
                        train_labels = train_labels[1:] 
                    else:
                        val_tensors = tensors[1:] 
                        affinity_matrix = self.calc_cosine(val_tensors, train_tensors)
                        val_labels = val_labels[1:] 
                
                    for i in range(affinity_matrix.shape[0]):
                        class_indices = torch.topk(affinity_matrix[i], k=self.conf.K, dim=0).indices
                        
                        class_indices = torch.topk(affinity_matrix[i], k=self.conf.K, dim=0).indices
                        class_label = torch.mode(train_labels[class_indices]).values
                        y_hat.append(class_label) 
                        
                  

                    y_hat = torch.stack(y_hat).to('cpu') 
                    
                    labels = val_labels if mode == 'val' else train_labels 
                  
                    
                    accuracy = accuracy_score(labels.cpu().numpy(), y_hat) 
                
                    self.logger.experiment.add_scalar(f'{mode}_accuracy', accuracy, self.current_epoch)
    ...
    def calc_sim_scores(self, z, epsilon, option1=False):
        """ 
        calc_sim_scores calculates the similarity scores between the embeddings and the prototypes

        Args:
                prototype_vectors (torch.tensor): the prototypes
                z (torch.tensor): the embeddings
                epsilon (float): the epsilon value for the similarity scores

        Output:
                sim_scores (torch.tensor): the similarity scores between the embeddings and the prototypes 
        """
        d = torch.cdist(z, self.model.prototype_vectors.to('cuda'), p=2) 
        sim_scores = torch.log((d + 1) / (d + epsilon))
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                losses = self.sampler.training_losses(model=self.model,
                                    x_start=x_start,
                                    t=t, labels=labels, K=self.conf.K,
                                    alpha=self.conf.alpha,
                                    epsilon=self.conf.epsilon, 
                                    slice=slice, current_epoch=self.current_epoch, load_in=self.conf.load_in) 
            ...
            normie = 0.00003 * torch.norm(torch.cat([param.data.view(-1) for (name, param) in self.model.named_parameters() if 'encoder' in name]), p=1) 

            loss = losses['mse'].mean() + losses['pred-loss']  + normie if self.current_epoch > self.conf.load_in else (losses['mse'].mean() * 100) + normie 
            
            if "contrastive-loss" in losses:
                loss += losses['contrastive-loss'] 
                self.logger.experiment.add_scalar('contrastive loss', losses['contrastive-loss'],
                                                  self.num_samples)
            ...
            if self.global_rank == 0:
                if self.current_epoch >= self.conf.load_in:
                    ...
                    self.logger.experiment.add_scalar('predictive loss', losses['pred-loss'],
                                                        self.num_samples)

                
                self.logger.experiment.add_scalar('L1 Norm', normie,
                                                self.num_samples) 
                
                self.logger.experiment.add_scalar('MSE', losses['mse'].mean(),
                                                    self.num_samples)  
            ...
            with torch.no_grad():
                ...
                for x_T in loader:
                    ...
                    if self.conf.train_mode.is_latent_diffusion(
                        ...
                        #print('Looks like we are just sampling m8')
                ...
                # X_T = torch.randn(200, 1, 28, 28).to('cuda')

                # _prototypes = self.render(noise=X_T, cond=self.model.prototype_vectors)

                
                # proto_grid = make_grid(_prototypes, n_rows=5) 
                # self.logger.experiment.add_image(f'sample{postfix}/prototypes', 
                #                                 proto_grid, self.num_samples) 

                # if not os.path.exists(f'sample{postfix}/prototypes'):
                #     os.makedirs(f'sample{postfix}/prototypes') 
                    
                # path = os.path.join(f'sample{postfix}/prototypes',
                #     '%d.png' % self.num_samples) 

                # save_image(proto_grid, path) 
    ...
    def configure_optimizers(self):
        ...
        # set up a reduce lr on plataeu scheduler
        # sched = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, mode='max', factor=0.1, patience=3, verbose=True, threshold=0.05, threshold_mode='abs', cooldown=0, min_lr=0, eps=1e-08)
        
        # out['lr_scheduler'] = {'scheduler': sched, 'monitor': 'Training Accuracy', 'interval': 'epoch', 'frequency': 1}
...
class SecondGroupScheduler:
    def __init__(self, optimizer, scheduler):
        self.optimizer = optimizer
        self.scheduler = scheduler

    def step(self):
        # Save the learning rate of the first parameter group
        old_lr = self.optimizer.param_groups[0]['lr']

        # Step the scheduler
        self.scheduler.step()

        # Restore the learning rate of the first parameter group
...
def is_time(num_samples, every, step_size, hijack=False, epoch=0):
    if hijack:
        if epoch % 5 == 0:
            return True
        else:
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    if conf.continue_from is not None:
        ...
        resume = conf.continue_from_path 
    elif conf.from_ckpt is not None and os.path.exists(checkpoint_path):
