...
class GaussianDiffusionBeatGans:
    ...
    def __init__(self, conf: GaussianDiffusionBeatGansConfig):
        self.prototype_class_identity = torch.zeros(70, 10)
    ...
    def training_losses(self,
        model, x_start: th.Tensor,t: th.Tensor,labels=None,
        alpha=0, _lambda=0, beta=0, epsilon = None, noise: th.Tensor = None, 
        model_kwargs=None, slice=None, contrast=False,tau=0.7, K=7,
        current_epoch=0, load_in=1000):
        """
        ...
        if self.loss_type in [
            ...
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                    if current_epoch >= load_in:
                        terms['pred-loss'] = pred_loss 
                        terms['contrastive-loss'] = contrastive_loss  * alpha  
    ...
    def calc_pred_loss(self, affinity_matrix, labels, network=None, aux=False, K=15, full=True): 
        #memory allocation for the predictions 
        y_hat = []
        
        #instantiate the loss 
        loss = nn.CrossEntropyLoss(reduction='mean')
         
        #loop through the rows of the affinity matrix  
        for i in range(affinity_matrix.shape[0]):
            #get the indices of the top K values
            class_indices = torch.topk(affinity_matrix[i], k=K, dim=0).indices
            #get the class of the mode of the top K values
            class_label = torch.mode(labels[class_indices]).values
            #append the prediction 
            y_hat.append(class_label)         

    
        
        y_pred = F.one_hot(torch.stack(y_hat).to(torch.int64), num_classes=2).to(torch.float32)
        
        l = loss(y_pred, labels.to(torch.long)) 
