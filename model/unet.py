...
from .nn import (conv_nd, linear, normalization, timestep_embedding,
                 torch_checkpoint, zero_module, GroupNorm32, SiLU)
...
class BeatGANsUNetModel(nn.Module):
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        self.output_struct = [elem for arr in input_block_chans for elem in arr] 
        repeat = False 
         
        if not self.old_out: 
            for i in reversed(range(1, len(self.output_struct))):
                layers = [
                        ResBlockConfig(
                            # only direct channels when gated
                            channels=self.output_struct[i],
                            emb_channels=conf.embed_channels,
                            dropout=conf.dropout,
                            out_channels= self.output_struct[i-1],
                            dims=conf.dims,
                            use_checkpoint=conf.use_checkpoint,
                ...
                if self.output_struct[i] == self.output_struct[i-1]: 
                    repeat += 1
                           
                if repeat == 2:
                    repeat = 0 
                    layers.append(
                            Upsample(self.output_struct[i],
                            conf.conv_resample,
                            dims=conf.dims,
                            out_channels=self.output_struct[i])) 
        ...
        if conf.resnet_use_zero_module: 
            ...
            self.out = nn.Sequential(
                normalization(ch),
                nn.SiLU(),
                conv_nd(conf.dims, 32, conf.out_channels, 3, padding=1),
            )
...
class BeatGANsEncoderModel(nn.Module):
    ...
    def forward(self, x, t=None, slice=None, eval=False, return_2d_feature=False):
        """
        Apply the model to an input batch.
        ...
        for idx, module in enumerate(self.input_blocks):
            if idx == 2 and slice is not None:
                h = apply_conditions(h, emb=slice, eval=eval)  
        ...
        if slice is not None:
            h = apply_conditions(h, emb=slice, layers=nn.Sequential(GroupNorm32(32, 512, eps=1e-05, affine=True),
                                            SiLU(),
                                            nn.Dropout(p=0.1, inplace=False),
                                            nn.Conv2d(512, 512, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))), middle=True, eval=eval) 
        ...
        h = self.out(h) 
        if slice is not None:
            h = th.cat([h, slice.to(th.float32).reshape(slice.shape[0], 1)], axis=-1) 
            if eval:
                h = nn.Sequential(nn.BatchNorm1d(513), nn.SiLU(), nn.Linear(513, 512)).eval().to('cuda')(h) 
            else:
                h = nn.Sequential(nn.BatchNorm1d(513), nn.SiLU(), nn.Linear(513, 512)).to('cuda')(h) 
...
def apply_conditions(
    ...
    cond=None,
    two_cond=False,
    layers: nn.Sequential = nn.Sequential(GroupNorm32(32, 128, eps=1e-05, affine=True),
                                          SiLU(),
                                          nn.Dropout(p=0.1, inplace=False),
...
middle=False, eval=False):
    ...
    layers = layers.to('cuda')
    emb = emb.to(th.float32).reshape(len(emb), 1) 

    if not middle: 
        if eval: 
            emb = nn.Sequential(nn.Linear(1, 32), nn.ReLU(), nn.BatchNorm1d(32), nn.Linear(32, 128)).to('cuda').eval()(emb)
        else:
            emb = nn.Sequential(nn.Linear(1, 32), nn.ReLU(), nn.BatchNorm1d(32), nn.Linear(32, 128)).to('cuda')(emb)
            
        emb = emb.reshape(len(emb), 128, 1, 1) 

    else:
        if eval:
            emb = nn.Sequential(nn.Linear(1, 32), nn.ReLU(), nn.BatchNorm1d(32), nn.Linear(32, 128), nn.ReLU(), nn.BatchNorm1d(128), nn.Linear(128, 512)).to('cuda').eval()(emb)
        
        else:
            emb = nn.Sequential(nn.Linear(1, 32), nn.ReLU(), nn.BatchNorm1d(32), nn.Linear(32, 128), nn.ReLU(), nn.BatchNorm1d(128), nn.Linear(128, 512)).to('cuda')(emb)
        
        emb = emb.reshape(len(emb), 512, 1, 1) 
